...
class NodeFeaExtractor(nn.Module):
    ...
    def forward(self, node_feat):
        t, n = node_feat.shape
        x = node_feat.transpose(1, 0).reshape(n, 1, -1)
        x = self.conv1(x)
        x = torch.relu(x)
        x = self.bn1(x)
        x = self.conv2(x)
...
class EvolvingGraphLearner(nn.Module):
    ...
    def forward(self, inputs: Tensor, states):
        ...
        # print(inputs.shape)
        dy_sent = torch.relu(inputs)
        dy_revi = dy_sent.transpose(1,2)
        support = torch.nn.functional.softmax(torch.nn.functional.relu(torch.matmul(dy_sent, dy_revi)))
