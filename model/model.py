...
class Graph_Baseline(BaseSeqModel):
    def __init__(self, skill_num, embed_dim, skill_embed_dim, class_num, nhead, nlayers, dropout, model, *args, **kwargs):
        ...
        self.inflow_MLP_1 = nn.Linear(embed_dim+skill_embed_dim, embed_dim)
        self.outflow_MLP_1 = nn.Linear(embed_dim+skill_embed_dim, embed_dim)
        self.inflow_decoder = nn.Linear(embed_dim, class_num)
        self.outflow_decoder = nn.Linear(embed_dim, class_num)
        self.inflow_MLP_2 = nn.Linear(embed_dim, embed_dim)
        self.outflow_MLP_2 = nn.Linear(embed_dim, embed_dim)
    ...
    def _decode(self, x, s):
        ...
        d_y = self.inflow_decoder(F.leaky_relu(self.inflow_MLP_2(self.drop(F.leaky_relu(self.inflow_MLP_1(torch.concat([d_x, s[:,:]], dim=-1)))))))
        s_y = self.outflow_decoder(F.leaky_relu(self.outflow_MLP_2(self.drop(F.leaky_relu(self.outflow_MLP_1(torch.concat([s_x, s[:, :]], dim=-1)))))))
...
class Skill_Evolve_Hetero(BaseSeqModel):
    ...
    def __init__(self, skill_num, embed_dim, skill_embed_dim, class_num, nhead, nhid, nlayers, dropout, sample_node_num, model, hyperdecode=False, hypcomb='', *args, **kwargs):
        ...
        elif model == "multi-view":
            self.largedyskillhgnn = MultipleGraph_Learning(skill_num, skill_embed_dim, sample_node_num,model)
        ...
        self.init_start_token = nn.Embedding(1, skill_embed_dim)
        self.aux_predict_1 = nn.Linear(embed_dim*2, embed_dim)
        self.aux_predict_2 = nn.Linear(embed_dim, 4)
        # decoder
        ...
        # self.before_hyp_MLP_out = nn.Sequential(
        #     nn.Linear(embed_dim*2, embed_dim),
        #     nn.Dropout(p=dropout),
        ...
        #     nn.BatchNorm1d(embed_dim),
        #     )
        self.inflow_MLP = nn.Sequential(
            nn.Linear(embed_dim, embed_dim),
            nn.Dropout(p=dropout),
            nn.LeakyReLU(),
            nn.Linear(embed_dim, class_num)
            )
        self.outflow_MLP = nn.Sequential(
            nn.Linear(embed_dim, embed_dim),
            nn.Dropout(p=dropout),
            nn.LeakyReLU(),
            nn.Linear(embed_dim, class_num)
        ...
        # self.fuse_hyper = nn.Linear(embed_dim * 1+skill_embed_dim*2, self.hyper_size)
        # self.fuse_hyper_orgemb = nn.Linear(embed_dim * 1+skill_embed_dim*2, embed_dim)
    ...
    def forward(self, x, l, s, t_s, t_e, g, comm, skill_semantic_embed, gap):
        ...
        # pred = 0
        # pred = self.aux_predict_2(F.relu(self.aux_predict_1(torch.cat([enc_d_x,enc_s_x],dim=-1))))
        # pred = F.softmax(pred, dim=-1)
        # # print(pred)
        # pred_in_hyper = torch.cat([pred,pred],dim=0)
        # embedding = embedding[s,:]
        # print(embedding.shape)
        # seq_len = t_e.view(-1,1,1).expand(embedding.size(0), 1, embedding.size(2))
        # print(torch.squeeze(s_emb.gather(1, seq_len)).shape)
        # print(embedding.shape)
        # print(enc_s_x[:,-1,:].shape)
        if self.hyperdecode:
    ...
    def _combined_decode(self, x, s):
        ...
        d_y = self.inflow_MLP(self.before_hyp_MLP(torch.concat([d_x, s[:,:]], dim=-1)))
        s_y = self.outflow_MLP(self.before_hyp_MLP(torch.concat([s_x, s[:, :]], dim=-1)))
        return F.log_softmax(d_y, dim=-1), F.log_softmax(s_y, dim=-1)
    def _decode(self, x, s):
        d_x, s_x = x
        # d_s = self.inoutflow_merge(torch.concat([d_x, s_x, s], dim=-1))
        d_y = self.inflow_MLP(self.before_hyp_MLP(torch.concat([d_x, s[:d_x.shape[0],:]], dim=-1)))
        s_y = self.outflow_MLP(self.before_hyp_MLP(torch.concat([s_x, s[d_x.shape[0]:, :]], dim=-1)))
        return F.log_softmax(d_y, dim=-1), F.log_softmax(s_y, dim=-1)
    def _hyperdecode(self, x, s, cond=None):
        ...
        if self.hypcomb == "res":
            d_y = d_x + d_y_h
            s_y = d_x + s_y_h
            d_y = self.inflow_MLP(d_y)
            s_y = self.outflow_MLP(s_y)
            ...
            d_y = self.before_hyp_MLP(torch.concat([d_x, s[:d_x.shape[0],:]], dim=-1))
            s_y = self.before_hyp_MLP(torch.concat([s_x, s[d_x.shape[0]:,:]], dim=-1))
            d_y = self.inflow_MLP(d_y) + self.inflow_MLP(d_y_h)
            s_y = self.outflow_MLP(s_y) + self.inflow_MLP(s_y_h)
...
class MultipleGraph_Learning(nn.Module):
    ...
    def __init__(self, skill_num, skill_embed_dim, sample_node_num, model):
        ...
        self.fuse_edge_seq =nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(skill_embed_dim, skill_embed_dim)
            )
        self.fuse_emb = nn.Linear(skill_embed_dim*2, skill_embed_dim)
        self.fuse_pool = nn.Linear(skill_embed_dim*2, 1)
        self.attention = torch.nn.MultiheadAttention(skill_embed_dim,num_heads=4,add_zero_attn=True)
        # self.fuse_seq_2 = nn.Linear(skill_embed_dim, skill_embed_dim)
        # self.multihop_linear = nn.Linear(skill_embed_dim, skill_embed_dim)
        self.mlp1 = normal_mlp(skill_embed_dim)
        self.mlp2 = normal_mlp(skill_embed_dim)
        self.loss_mse = nn.MSELoss()
        self.L = torch.nn.L1Loss()
    ...
    def forward(self, inflow_seq_emb, outflow_seq_emb, l, t_s, t_e, g_d, comm, skill_semantic_embed, init_emb):
        ...
        inflow_emb_1 = self.graph_emb.weight
        outflow_emb_1 = self.graph_emb.weight

        # inflow_emb_2 = self.skill_inflow_LT(self.graph_emb.weight)
        # outflow_emb_2 = self.skill_outflow_LT(self.graph_emb.weight)
        cat = torch.cat([inflow_emb_1,outflow_emb_1], dim=0)
        # cat_2 = torch.cat([inflow_emb_2,outflow_emb_2], dim=0)
...
class GCN(nn.Module):
    ...
    def __init__(self, skill_embed_dim: int):
        ...
        self.GCN_1 = geo_nn.conv.GCNConv(skill_embed_dim, skill_embed_dim, dropout=0.2, normalize=True)
        self.GCN_2 = geo_nn.conv.GCNConv(skill_embed_dim, skill_embed_dim, dropout=0.2, normalize=True)
